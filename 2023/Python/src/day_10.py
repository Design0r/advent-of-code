...
class Grid:
    def __init__(self, input: list[str]):
        ...
        self.possible_symbols = {"UP": ("|", "7", "F", "S"), "DOWN": ("|", "L", "J", "S"), "LEFT": ("-", "L", "F", "S"),
                                 "RIGHT": ("-", "7", "J", "S")}
        self.possible_dirs = {"|": ("UP", "DOWN"), "-": ("LEFT", "RIGHT"), "L": ("UP", "RIGHT"), "J": ("UP", "LEFT"),
    ...
    def is_possible_move(self, direction: str, current: tuple[int, int], next: tuple[int, int]) -> bool:
        ...
        if 0 <= n_y < len(self.grid) and 0 <= n_x < len(self.grid[0]):
            ...
            if next_symbol in possible_symbols and direction in self.possible_dirs.get(curr_symbol,
                                                                                       ("UP", "DOWN", "LEFT", "RIGHT")):
                return True
            return False
